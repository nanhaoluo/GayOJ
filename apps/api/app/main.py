...
def contest_user_is_participant(user: User, contest: Contest, store: Repository) -> bool:
    if user.role != "student":
        return False
    if contest.participation_mode == "open":
        return True
...
def contest_detail(contest: Contest, store: Repository, *, viewer: User | None = None) -> ContestDetail:
...
def contest_owners(store: Repository, contest: Contest) -> set[str]:
    owners: set[str] = set(contest_participant_user_ids(contest, store))
...
def _build_score_standings(
    ...
    *,
    ...
    participant_ids = {submission.user_id for submission in submissions}
    rows: dict[str, dict[str, Any]] = {
        user_id: {
            "user_id": user_id,
            "display_name": users.get(user_id).display_name if users.get(user_id) else user_id,
            "solved": 0,
            "score": 0,
            "penalty": 0,
            "first_blood": 0,
...
def list_contests(
    ...
    if not user:
        ...
        contests = [contest for contest in contests if contest.visibility == "public"]
    return [contest_detail(contest, store, viewer=user) for contest in contests]
