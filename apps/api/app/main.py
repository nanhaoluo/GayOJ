...
def contest_problem_layout(contest: Contest) -> list[ContestProblemLayoutItem]:
    if contest.problem_layout:
        return contest.problem_layout
    return [
        ContestProblemLayoutItem(problem_id=problem_id, problem_key=str(index + 1), allowed_languages=[])
        for index, problem_id in enumerate(contest.problem_ids)
    ]
...
def ensure_contest_problem_language_allowed(
    contest: Contest,
    problem: Problem,
    language: str | None,
) -> None:
