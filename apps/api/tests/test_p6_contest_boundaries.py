...
def test_contest_individual_registration_limits_problem_submit_clar_and_print(client: TestClient, auth_headers, store) -> None:
    ...
    listed = client.get("/api/v1/contests", headers=auth_headers("alice"))
    detail = client.get("/api/v1/contests/C1001", headers=auth_headers("alice"))
    assert listed.status_code == 200, listed.text
    assert any(item["id"] == "C1001" and item["self_registered"] is False for item in listed.json())
    assert detail.status_code == 200, detail.text
    assert detail.json()["self_registered"] is False
