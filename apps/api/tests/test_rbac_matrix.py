...
def test_admin_can_query_rbac_matrix(client: TestClient, auth_headers) -> None:
    ...
    assert "submission:create" not in roles["coach"]
    assert "training:offline" not in roles["coach"]
    assert "clarification:create" not in roles["coach"]
    assert "submission:override" in roles["judge"]
    assert "judge:monitor" in roles["judge"]
    assert "submission:create" not in roles["judge"]
    assert "clarification:create" not in roles["judge"]
    assert "submission:override" in roles["admin"]
    assert "judge:monitor" in roles["admin"]
    assert "submission:create" not in roles["admin"]
    assert "training:offline" not in roles["admin"]
    assert "clarification:create" not in roles["admin"]
